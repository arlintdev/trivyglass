...
<div>
	...
	<div
		style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: var(--space-md);"
	>
		{#each sortedChecks as check}
			<div class="nd-card" style="display: flex; flex-direction: column; gap: var(--space-sm);">
				<div style="display: flex; align-items: center; justify-content: space-between;">
					...
					<span class="nd-tag nd-tag-{severityTag(check.severity)}"
						>{check.severity ?? 'UNKNOWN'}</span
					>
				</div>
				<div style="display: flex; flex-direction: column; gap: var(--space-sm);">
					...
					<div class="nd-alert nd-alert-error">
						<strong>[ISSUE]</strong>
						{check.messages?.[0] ?? 'No specific issue reported'}
					</div>
					<div
						style="border-left: 2px solid var(--nd-border-visible); padding-left: var(--space-md);"
					>
