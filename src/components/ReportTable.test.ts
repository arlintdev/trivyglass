...
describe('ReportTable Component', () => {
  ...
  it('filters reports based on search term', async () => {
    ...
    const searchInput = document.querySelector('input[placeholder="Search reports"]') as HTMLInputElement;
  ...
  it('generates correct links for reports', () => {
    ...
    expect(detailsButtons[0].getAttribute('href')).toBe('/vulnerability/namespace/default/test-report-1');
    expect(detailsButtons[1].getAttribute('href')).toBe('/vulnerability/namespace/kube-system/test-report-2');
