...
export function handleConnectionError(error: unknown): ConnectionErrorType {
	...
	switch (errorType) {
		case ConnectionErrorType.CLUSTER:
			toastStore.addToast(`Cluster connection error: ${errorMessage}`, 'error', 7000);
			break;
		case ConnectionErrorType.REDIS:
			toastStore.addToast(`Cache connection error: ${errorMessage}`, 'warning', 7000);
			break;
		case ConnectionErrorType.OTHER:
			toastStore.addToast(`Application error: ${errorMessage}`, 'info', 5000);
			break;
	}
