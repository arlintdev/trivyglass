<script lang="ts">
	...
	import { ChevronDownOutline, ServerSolid, ExclamationCircleSolid, CogSolid } from 'flowbite-svelte-icons';
	...
	let clusterManagerStatus = $state(false);
			...
			if (!recentlyChanged) {
				...
				if (currentCluster !== data.currentCluster) {
					...
					if (storedCluster && storedCluster !== data.currentCluster) {
						console.warn(`API cluster (${data.currentCluster}) doesn't match localStorage (${storedCluster})`);
	...
	onMount(() => {
		...
		window.addEventListener('storage', (event) => {
			if (event.key === 'currentCluster' || event.key === 'clusterSwitched' ||
	...
	$effect(() => {
		...
		navStatus = nav.isOpen;
		clusterManagerStatus = clusterManager.isOpen;
		...
		{#snippet navSlotBlock()}
			<div class="flex items-center space-x-1 md:order-2">
				<div class="flex items-center mr-2">
					{#if connectionError}
						<div class="text-red-500 flex items-center mr-2" title={connectionError}>
							<ExclamationCircleSolid class="h-5 w-5" />
						</div>
					{/if}
					<div class="relative flex items-center">
						<div class="text-sm font-medium hidden md:block mr-2">
						...
						<div class="relative mr-2">
							...
							>
								<ServerSolid class="h-5 w-5 mr-1" />
							...
							<Dropdown
								...
								class="absolute top-[40px] right-0 z-50 w-56"
							>
								<DropdownHeader>
								...
								<DropdownUl class="py-2">
									...
									{:else}
										{#each clusters as cluster}
											<DropdownLi
												class="cursor-pointer {currentCluster === cluster.name ? 'bg-gray-100 dark:bg-gray-700' : ''}"
											>
												<button
													class="w-full text-left px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600"
													on:click={() => switchCluster(cluster.name)}
												>
												<div class="flex items-center">
													<ServerSolid class="mr-2 h-4 w-4 text-gray-500" />
													<span>{cluster.name}</span>
													{#if currentCluster === cluster.name}
														<span class="ml-2 rounded-full bg-blue-100 px-2 py-0.5 text-xs text-blue-800 dark:bg-blue-900 dark:text-blue-300">
															Active
														</span>
													{/if}
												</div>
						...
						<Button
							color="gray"
							class="p-2 cluster-manager-button"
