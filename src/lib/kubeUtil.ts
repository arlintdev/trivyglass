...
class RedisCache implements Cache {
	...
	async hGet(hash: string, field: string): Promise<string | undefined | null> {
		if (!this.connected) return null;
...
export async function listClusters(): Promise<ClusterInfo[]> {
	...
	try {
		...
		if (!cache.sMembers) {
			Logger.warn(
				'Cache implementation does not support set operations, returning only local cluster'
			);
...
function getColumnDefinitions(versionInfo: unknown): ColumnDefinition[] {
	...
	return (
		(versionInfo as { additionalPrinterColumns?: unknown[] }).additionalPrinterColumns?.map(
			(col: unknown) => ({
...
export async function listAllAquaSecurityCRDs(): Promise<CRDMetadata[]> {
	...
	try {
		...
		const crds = items
			.filter(
				(crd: unknown) =>
					(crd as { spec: { group: string; scope: string } }).spec.group ===
		...
		Logger.error(
			`Critical error fetching Aqua CRDs: ${err instanceof Error ? err.message : String(err)}`
		);
		return [];
	}
}
...
export async function loadReports(crdPlural: string): Promise<ReportsData> {
	...
	try {
		...
		Logger.error(
			`Critical error fetching ${crdPlural}: ${err instanceof Error ? err.message : String(err)}`
		);
		const data: ReportsData = {
			manifests: [],
...
export async function getNamespacedResource(
	...
	try {
		...
		try {
			...
			// Handle API errors
			Logger.error(
				`API Error getting namespaced resource ${resource}/${name} in ${namespace}: ${apiErr instanceof Error ? apiErr.message : String(apiErr)}`,
				apiErr
			);
			...
			if (apiErr && typeof apiErr === 'object' && 'response' in apiErr) {
				...
				if (apiError.response && apiError.response.body) {
					try {
						...
						const responseBody =
							typeof apiError.response.body === 'string'
		...
		Logger.error(
			`Error getting namespaced resource ${resource}/${name} in ${namespace}: ${err instanceof Error ? err.message : String(err)}`,
			err
		);
		return {
			manifest: null,
...
export async function getClusterResource(
	...
	try {
		...
		try {
			...
			// Handle API errors
			Logger.error(
				`API Error getting cluster resource ${resource}/${name}: ${apiErr instanceof Error ? apiErr.message : String(apiErr)}`,
				apiErr
			);
		...
		// Handle other errors (like connection issues)
		Logger.error(
			`Error getting cluster resource ${resource}/${name}: ${err instanceof Error ? err.message : String(err)}`,
			err
		);
		return {
			manifest: null,
