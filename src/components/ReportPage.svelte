...
<div class="container mx-auto p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
    ...
    <div class="p-4 bg-red-50 rounded-t-lg dark:bg-red-800">
      ...
      <p class="whitespace-normal break-words"><strong>Name:</strong> {data.manifest.metadata.name ?? 'N/A'}</p>
      <p class="whitespace-normal break-words"><strong>Namespace:</strong> {data.manifest.metadata.namespace ?? 'N/A'}</p>
      <p class="whitespace-normal break-words"><strong>Created:</strong> {data.manifest.metadata.creationTimestamp ? new Date(data.manifest.metadata.creationTimestamp).toLocaleString() : 'N/A'}</p>
    </div>
	</Card>
  ...
  <Card class="border border-gray-200 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 dark:border-gray-700">
    ...
    <div class="p-4 space-y-2 text-gray-700 dark:text-gray-300">
    <p class="whitespace-normal break-words"><strong>Name:</strong> {data.manifest?.report?.scanner?.name ?? 'N/A'}</p>
    <p class="whitespace-normal break-words"><strong>Vendor:</strong> {data.manifest?.report?.scanner?.vendor ?? 'N/A'}</p>
    <p class="whitespace-normal break-words"><strong>Version:</strong> {data.manifest?.report?.scanner?.version ?? 'N/A'}</p>
    </div>
  </Card>
  {/if}
  <!-- Registry Card-->
  ...
  <Card class="border border-gray-200 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 dark:border-gray-700">
    <div class="p-4 bg-blue-50 rounded-t-lg dark:bg-blue-900">
      <h2 class="text-xl font-bold text-blue-800 dark:text-blue-200">Registry Details</h2>
    </div>
    <div class="p-4 space-y-2 text-gray-700 dark:text-gray-300">
      <p class="whitespace
      -normal break-words"><strong>Server:</strong> {data.manifest.report.registry.server ?? 'N/A'}</p>
      </div>
  </Card>
  {/if}
  ...
  {#if data.manifest?.report?.summary}
	<Card class="border border-gray-200 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 dark:border-gray-700">
      ...
      <Badge class="bg-red-500 text-white dark:bg-red-600 dark:text-white">Critical: {data.manifest.report.summary.criticalCount ?? 0}</Badge>
      <Badge class="bg-orange-500 text-white dark:bg-orange-600 dark:text-white">High: {data.manifest.report.summary.highCount ?? 0}</Badge>
      <Badge class="bg-yellow-400 text-gray-800 dark:bg-yellow-600 dark:text-gray-200">Medium: {data.manifest.report.summary.mediumCount ?? 0}</Badge>
      <Badge class="bg-green-500 text-white dark:bg-green-600 dark:text-white">Low: {data.manifest.report.summary.lowCount ?? 0}</Badge>
		</div>
	  </div>
	</Card>
  {/if}
...
<Card class="border border-gray-200 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 dark:border-gray-700">
  ...
  <div class="p-4 flex flex-col gap-2 text-gray-700 dark:text-gray-300">
    <p class="whitespace-normal break-words"><strong>Digest:</strong> {data.manifest.report.artifact.digest ?? 'N/A'}</p>
    <p class="whitespace-normal break-words"><strong>Repository:</strong> {data.manifest.report.artifact.repository ?? 'N/A'}</p>
    <p class="whitespace-normal break-words"><strong>Tag:</strong> {data.manifest.report.artifact.tag ?? 'N/A'}</p>
  </div>
</Card>
{/if}
...
<Card class="border border-gray-200 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 dark:border-gray-700">
  ...
  <div class="p-4 flex flex-col gap-2 text-gray-700 dark:text-gray-300">
    <p class="whitespace
    -normal break-words"><strong>Family:</strong> {data.manifest.report.os.family ?? 'N/A'}</p>
    <p class="whitespace-normal break-words"><strong>Name
    :</strong> {data.manifest.report.os.name ?? 'N/A'}</p>
  </div>
</Card>
{/if}
